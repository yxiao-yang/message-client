...
MessageWindowLstItemWgt::MessageWindowLstItemWgt(QWidget* parent) 
	: QWidget(parent)
{
    setStyle();
}

void MessageWindowLstItemWgt::setStyle()
{
    // 设置字体
...
void MessageWindowLstItemWgt::setTextSuccess()
{
    m_lbLoading->hide();
    m_movieLoading->stop();
    m_isSending = true;
...
void MessageWindowLstItemWgt::setText(QString text, QString time, QSize allSize, MessageWindowLstItemWgt::User_Type userType)
{
    m_strMsg = text;
    m_typeUser = userType;
    m_strTime = time;
    ...
    m_strCurTime = time;
    m_strCurTime.remove(0, 5);
    m_strCurTime.remove(-10, 10);
    m_allSize = allSize;
    if (userType == User_Me)
    {
        if (!m_isSending)
...
void MessageWindowLstItemWgt::paintEvent(QPaintEvent* event)
{
    Q_UNUSED(event);
    ...
    else if (m_typeUser == User_Type::User_Time)
    {
        // 时间
        QPen penText;
        ...
        painter.drawText(this->rect(), m_strCurTime, option);
    }
}

QSize MessageWindowLstItemWgt::getRealString(QString src)
{
...
QSize MessageWindowLstItemWgt::fontRect(QString str)
{
    m_strMsg = str;
    int minHei = 30;
    int iconWH = 40;
